...
_STREAMABLE_HTTP_ACCEPTS_HTTP_CLIENT = (
    "http_client" in signature(streamable_http_client).parameters
)
...
class MCPClient:
    ...
    async def _connect_http(self) -> tuple[Any, Any]:
        ...
        if _STREAMABLE_HTTP_ACCEPTS_HTTP_CLIENT:
            ...
            result = await self._stack.enter_async_context(
                streamable_http_client(self.config.url, headers=resolved_headers)
            )
