...
def build_time_gap_message(last_active: datetime) -> Message | None:
    """Return a resume reminder when a session has been idle long enough."""
    now = datetime.now(timezone.utc)
    if last_active.tzinfo is None:
        last_active = last_active.replace(tzinfo=timezone.utc)

    elapsed = now - last_active.astimezone(timezone.utc)
    ...
    if days >= 1:
        ...
        gap_text = f"{max(1, int(elapsed.total_seconds() // 3600))} 小时"

    return Message(
        role="user",
        content=(
            "距离上次会话已经过去约 "
            f"{gap_text}。期间代码可能有变更，请先检查当前文件状态，"
            "再基于最新代码继续。"
        ),
    )
